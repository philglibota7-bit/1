...
**Die Crew sieht nach und zeigt.** Ein eigener Agent sah bisher nur seinen Auftrag. Jetzt hat er acht Werkzeuge und kann damit an der Station nachsehen, dem Nutzer etwas zeigen und, wenn er darum gebeten wird, eine Notiz schreiben, eine Erinnerung stellen oder eine Figur beobachten: „Wer wartet gerade auf mich?“, „Was macht webshop-kasse, und wie weit ist sie?“, „Wie viel haben wir im Fenster verbraucht?“, „Zeig mir Tokio“.
- `station_lage` liest den Stand der Station als kleines JSON: jede Figur mit Art, Zustand, Tätigkeit, Ort, Zweig, seit wann, Aufgaben („3 von 5 erledigt“), Konflikt, Kreis, Notiz und Wache. Dazu kommen, wer wartet, die Konflikte, die Zeilen der Energie, der Frachter, das Zuhause, worüber die Station fliegt, Erinnerungen, Wachen und die letzten Einträge des Bordbuchs. Beispieldaten der Vorführung sind als solche markiert, mit einem Satz dazu.
...
- `tagesbogen` liest, woran heute (`heute`) oder in den letzten sieben Tagen (`woche`) gearbeitet wurde, derselbe Text wie „Tag kopieren“ und „Woche kopieren“ in der Brücke: heute je Projekt die Sitzungen mit Zeitspanne und womit sie begannen, die Woche je Tag die Projekte mit offener Zeit und am Ende die Summen. Damit antwortet die Crew auf „Woran habe ich heute gearbeitet?“ oder „Wohin ging die Woche?“. Beides kennt nur das Mac-Programm, das die Protokolle liest, und die Vorführung; im Browser bekommt Claude statt des Texts den Satz, dass es ihn hier nicht gibt.
...
- `erinnern` stellt eine Erinnerung in 1 bis 1440 Minuten, auf Wunsch an einer Figur; `beobachten` setzt eine Wache an eine Figur (fertig, wartet oder Fehler); `notiz` schreibt eine Notiz an eine Figur oder löscht sie mit leerem Text (siehe unten).
- Die Werkzeuge lesen nur, was ORBIT ohnehin zeigt, und wirken sonst nur auf die Ansicht: Erde, Kamera, Detailfenster. Ändern können sie nur Notizen, Erinnerungen und Wachen, und die Anleitung sagt ausdrücklich: nur, wenn der Nutzer selbst darum bittet. Sonst wird nichts geändert oder gelöscht. Findet ein Werkzeug nichts, bekommt Claude das als Fehler zurück („Keine Figur heißt …“) und arbeitet weiter. Ein Ergebnis ist höchstens 32 KB groß, die Übersicht höchstens 40 Figuren. Die Anleitung dazu sagt ihm, dass Ergebnisse Daten sind und keine Anweisungen.
...
- In der Vorschau gehen die Werkzeuge über den Betrachter (`tools` von `sample`), wenn er sie ausführen kann; sonst arbeitet die Crew ohne sie. Erlaubt er weniger Werkzeuge, als es gibt, bekommt sie die wichtigsten, der Reihe nach: `station_lage`, `figur_detail`, `zeige_figur`, `tagesbogen`, `zeige_ort`, `erinnern`, `beobachten`, `notiz`. Vorher hieß „zu wenige erlaubt“ gar keine, und ein fünftes Werkzeug hätte in so einem Betrachter die anderen vier gekostet.
...
- In der Szene sieht man, wen er gerade liest. Von seiner Figur läuft eine Funkstrecke in seiner Anzugfarbe zu jeder Figur, die ein Werkzeug angesehen hat: erst laufen die Punkte hin, solange er fragt, dann mit der Antwort zurück, und wo die Frage ankommt, geht ein Ring auf. `station_lage` ist ein Rundruf an alle, die nächste Figur zuerst, im Abstand von 0,11 Sekunden; `figur_detail` und `zeige_figur` eine Strecke. Nach gut zwei Sekunden ist sie wieder weg. Wie die Strecke zum Unteragenten liegt sie über der Szene, nicht in ihr: Funk sieht man im All nicht.

**Was fragen?** Tippt man ins leere Auftragsfeld, stehen darüber bis zu drei Fragen, die zur Lage passen, und der Punkt davor hat deren Farbe:
- Wartet eine Sitzung: „Worauf wartet webshop-kasse?“, bei mehreren „Wer wartet auf mich, und worauf?“ (bernstein).
- Dreht sich einer im Kreis: „Warum scheitert Aufrufe berechnePreis immer wieder?“ (orange).
- Ändern zwei dieselbe Datei: „Zwei ändern preis.ts – was tun?“ (rot). Zuerst standen die Namen beider darin; auf dem Handy war die Frage abgeschnitten, bevor sie fragte.
- Hat der Frachter geliefert: „Was hat der Frachter gebracht?“.
- Danach allgemeine: „Was ist gerade los an der Station?“, „Woran habe ich heute gearbeitet?“ (nur, wenn es heute Sitzungen gab), „Wie viel haben wir im Fenster verbraucht?“ (nur, wenn es Zahlen dazu gibt), „Zeig mir Tokio.“
- Ein Tipp setzt die Frage ins Feld, das Feld behält den Fokus, und am Handy bleibt die Tastatur offen. Abgeschickt wird erst mit dem Knopf: Jede Antwort kostet, und an die ganze Crew sind es drei. Ändert sich die Lage, während die Reihe offen ist, zieht sie nach, baut aber nur neu, was sich geändert hat, damit ein Tipp nicht ins Leere geht.
- Die Maßstabsleiste unten links steht jetzt über der Fußleiste, wie hoch die auch ist. Vorher hatte sie einen festen Abstand vom unteren Rand und lag, sobald die Fragen erschienen, auf dem ersten Knopf der Crew.

**Funk zum Hören.** Unter Stationswerte → FUNK, aus als Vorgabe. Vier Klänge, nicht mehr:
- Antwortet ein eigener Agent, beginnt und endet sein Funkspruch mit den Quindar-Tönen der Apollo-Flüge: 2525 Hz, wenn der erste Text kommt, 2475 Hz am Ende, je eine Viertelsekunde, reiner Sinus. Mit ihnen schaltete die Bodenkontrolle die fernen Sender ein und aus. Antwortet die ganze Crew zugleich, piepst es einmal, nicht dreimal. Eine Sitzung, die mit ihrem Zug fertig ist, bekommt den Ton vom Ende.
- Ruf, zwei weiche Töne aufwärts (880 und 1175 Hz): Eine Sitzung wartet auf dich, ein Lauf wartet auf eine Freigabe oder ein PR soll geändert werden.
- Achtung, zwei tiefere abwärts (523 und 392 Hz): ein Konflikt, ein Kreis, ein Fehler eines eigenen Agenten.
- Gut, eine Quinte aufwärts (659 und 988 Hz): Ein PR ist freigegeben, der Frachter hat geliefert.
- Jeder Ton wird in 8 ms ein- und in 12 ms ausgeblendet; ein harter Beginn knackt im Lautsprecher. Einer je Figur und Anlass in der Minute, wie bei den Mitteilungen, und die erste Sichtung klingt nicht.
- Ein Browser spielt erst nach einem Klick einen Ton. Beim Einschalten ist das der Klick auf den Schalter; zur Probe kommt der Ton vom Ende. Nach dem Neuladen wartet der Funk auf den ersten Klick oder Tastendruck irgendwo und spielt bis dahin nichts nach. Ein Stapel alter Töne auf einen Schlag wäre schlimmer als keiner.

**Die Crew spricht.** Der zweite Schalter unter FUNK, ebenfalls aus als Vorgabe. Dann liest jeder eigene Agent den Anfang seiner Antwort vor, wie einen Funkspruch: bei hörbarem Funk der Quindar-Ton, „Hier Kepler.“ und der erste Satz, dann der Ton vom Ende. Den Rest liest man im Log; eine ganze Antwort vorzulesen dauert Minuten.
- Vorgelesen wird ohne Code, Adressen und Auszeichnungen. Getrennt wird nur, wo nach dem Punkt Platz ist, also bleiben „preis.ts“ und „3.5“ ganz; ist der erste Satz kürzer als 60 Zeichen, kommt der nächste dazu, höchstens 200 Zeichen.
- Nur mit einer deutschen Stimme des Geräts, lieber einer eingebauten als einer aus dem Netz, lieber de-DE. Eine englische Stimme, die deutsche Sätze liest, versteht niemand; dann bleibt es still, die Töne kommen wie ohne Sprache, und der Schalter sagt „keine deutsche Stimme auf diesem Gerät“. Eingeschaltet nennt er die Stimme.
- Antwortet die ganze Crew, sprechen sie der Reihe nach, nie zwei zugleich, jeder zwischen seinen eigenen Tönen und jeder mit etwas anderer Tonhöhe: gleiche Stimme, drei Leute am selben Funkgerät. Höchstens drei warten. Schaltet man aus, bricht die Stimme mitten im Satz ab, und die Warteschlange ist leer.
- Beim Anfangston für einen gesprochenen Spruch gilt die Sperre von anderthalb Sekunden nicht. Der Test fand das: Bei kurzen Sprüchen fehlte dem zweiten Agenten der Anfangston, weil der erste gerade erst geklungen hatte.

**Notizen und Erinnerungen.** Was man selbst über eine Figur weiß, hatte bisher keinen Platz: „wartet auf Review von Mia“, „nach dem Essen mergen“. Und wer in zwanzig Minuten nach dem Build sehen wollte, brauchte dafür ein anderes Programm.
- Im Detailfenster jeder Figur steht oben NOTIZ, ein Feld für eine Zeile (80 Zeichen). Enter oder Wegtippen speichert, Esc verwirft, ✕ löscht. Das Fenster wird jede Sekunde angeglichen; wer darin schreibt, verliert dabei weder Text noch Fokus. Für welche Figur geschrieben wird, merkt sich das Feld beim Betreten: Zeigt das Fenster während des Schreibens eine andere Figur, weil die Crew sie zeigt, landet der Text trotzdem bei der ersten.
- Die Notiz hängt an der Figur, nicht an der Stelle: Geht eine Sitzung und kommt dieselbe wieder, steht ihre Notiz wieder da. Mehr als 200 Notizen hebt ORBIT nicht auf, dann fallen die ältesten weg.
- Die Notiz steht unter dem Namen in der Szene („✎ wartet auf Review · 2/8“), statt des Branch, weil man sie mit Absicht geschrieben hat. Auch wenn es eng ist und die übrigen Unterzeilen wegfallen, bleibt sie stehen. Die Befehlspalette zeigt sie und findet die Figur über sie, und die Crew liest sie in `station_lage` und `figur_detail`.
- Darunter ERINNERN: in 15 Minuten, in einer oder in drei Stunden. Der Text ist die Notiz, ohne Notiz „Nach webshop-kasse sehen“. Was für die Figur ansteht, steht dort mit Uhrzeit und ✕, alles zusammen unter Stationswerte bei ERINNERUNGEN.
- Fällig heißt: ein Band oben in der Mitte, bernsteinfarben wie „wartet auf dich“, mit Zeit, Text, der Figur, „+10 min“ und „Erledigt“. Hinter dem Namen der Figur steht ◷, bis sie erledigt ist, und ihre Kennleuchte an der Schulter, sonst nur nachts an, blinkt satt bernsteinfarben in dunkler Fassung, einmal je Sekunde, bei Tag und bei Nacht; wer im System weniger Bewegung eingestellt hat, sieht sie ruhig leuchten. Die bemerkt man aus dem Augenwinkel, auch wenn das Band gerade von einer Blende verdeckt ist. Dazu ein Eintrag im Bordbuch, eine Mitteilung, wenn ORBIT nicht vorne ist und Mitteilungen an sind, und bei hörbarem Funk der Ruf. Das Band steht außerhalb der übrigen Bedienung und ist deshalb auch im Kinomodus und über einer offenen Blende zu sehen. Auf dem Handy bricht es um, und läuft die Vorführung, steht es unter ihrem Band statt darauf.
- Hat ein Agent der Crew die Erinnerung gestellt und spricht die Crew, sagt er sie selbst an („Hier Kepler. Erinnerung: Build prüfen.“), zwischen seinen Quindar-Tönen und ohne Ruf dazu.
- Höchstens 20 warten zugleich; die 21. lehnt ORBIT ab, mit einem Satz, den auch die Crew versteht. Eine Mitteilung ohne Figur öffnet beim Antippen kein leeres Detailfenster.
- Alles bleibt auf diesem Gerät, bei den anderen Einstellungen. Eine Erinnerung läuft nur, solange ORBIT offen ist, denn ein Browser weckt keine geschlossene Seite. Im Hintergrund kann sie bis zu einer Minute später kommen, weil Browser Zeitgeber dort drosseln. War ORBIT zu, meldet sie sich beim nächsten Öffnen einmal, mit der Zeit, zu der sie fällig war („Erinnerung (fällig war 14:30): …“). Eine, die schon im Band stand, steht nach dem Neuladen gleich wieder dort, ohne zweiten Eintrag im Bordbuch.

**Die Wache.** „Sag mir Bescheid, wenn webshop-kasse fertig ist.“ Bisher meldete ORBIT das nur, wenn es nicht vorne war und Mitteilungen an waren, und dann für jede Sitzung. Wer vor dem Bildschirm auf eine bestimmte wartet, sah nichts.
- Im Detailfenster steht unter ERINNERN jetzt MELDEN, mit dem, was die Figur kann: Sitzungen „wenn fertig“ und „wenn sie wartet“ (aus dem Sitzungsregister von Claude Code: arbeitet, wartet, fertig), Unteragenten und eigene Agenten „wenn fertig“ und „bei einem Fehler“, ein Befehl nur „wenn beendet“ (er geht von der Station), ein GitHub-Repo alle drei (die Läufe sind durch, eine Freigabe wartet, rot). Ein Projektordner hat keine Zeile: Er ist nie fertig.
- Antippen stellt die Wache (◉, gedrückt), noch einmal hebt sie auf. Sie steht auch unter Stationswerte bei ERINNERUNGEN, mit ◉ statt einer Uhrzeit und ✕.
- Gezählt wird ein Wechsel, kein Zustand. Ist eine Sitzung schon fertig, wenn man die Wache stellt, meldet sie sich, wenn sie wieder gearbeitet hat und fertig ist; sonst käme die Meldung sofort. Nach dem Neuladen gilt der erste gelesene Stand als Ausgangspunkt, denn was in der Pause geschah, hat niemand gesehen.
- Gemeldet wird genau einmal, wie eine fällige Erinnerung: Band oben („WACHE · 14:32“, „webshop-kasse ist fertig“), ◷ am Namen, Bordbuch, Mitteilung und Ruf, oder die Stimme des Agenten, der sie gestellt hat. Danach ist die Wache vorbei. Die allgemeine Meldung desselben Wechsels fällt dann weg: ein Ereignis, ein Ton, eine Mitteilung.
- Die Crew stellt sie mit `beobachten`. Kann die Figur das nicht („bei einem Fehler“ an einer Sitzung, ein Projektordner), kommt ein Satz zurück, der sagt, was geht. Höchstens 20 stehen zugleich; eine, die einen Tag lang nichts gesehen hat, endet mit einem Satz im Bordbuch.

**Während du weg warst.** Ein Leitstand läuft weiter, wenn niemand hinsieht. Wer nach einer Pause zurückkam, fand die Antwort auf „Was ist passiert?“ nur im Bordbuch, zwischen Sonnenaufgängen, Ventilen und Einschlägen.
- Kommt man nach mindestens zehn Minuten zurück, steht rechts oben eine Karte: „WÄHREND DU WEG WARST · 50 MIN · 5 EREIGNISSE“ und darunter, in der Reihenfolge, in der es geschah, mit Uhrzeit und Farbpunkt, was die Arbeit betrifft: fertig, alle Aufgaben erledigt, berichtet und freigegeben (grün), wartet, Reserve aufgebraucht und über der Einsatzdauer (bernstein), Fehler, Konflikte, Kreise, rot und Änderungen gewünscht (rot), Lieferungen (blau), fällige Erinnerungen und Wachen, neue und beendete Sitzungen. Die Station selbst bleibt im Bordbuch.
- Sind es mehr als acht, stehen die sieben jüngsten da und darüber, wie viele frühere im Bordbuch stehen. „Ganzes Bordbuch“ öffnet es, ✕ schließt die Karte. Neun Minuten weg oder nur Station: keine Karte.
- Weg heißt: das Fenster ist nicht vorne oder die Seite nicht zu sehen; zurück heißt, beides wieder. Wurde ORBIT im Hintergrund geöffnet, zählt es von da an als weg.
- Was während der Abwesenheit geschieht, merkt sich ORBIT gesondert, bis zu 300 Einträge. Das Bordbuch selbst behält nur vierzig, und in ein paar Stunden füllen die allein Sonnenaufgänge, Ventile und Funk; eine Sitzung, die am Anfang fertig wurde, wäre sonst schon vergessen.
- Steht gerade das Band einer Erinnerung oder der Vorführung, steht die Karte darunter, nicht darauf. Auf dem Handy beginnt ein offenes Fenster unter Band und Karte; vorher lag das Band dort auf dem Kopf des Fensters, samt dem Knopf zum Schließen (behoben schon für Version 32).
- Die Crew liest in `station_lage` jetzt auch die letzten 15 Einträge des Bordbuchs, neueste zuerst. Steht die Karte, steht über dem leeren Auftragsfeld „Was ist passiert, während ich weg war?“.

**Der Schichtbericht.** Wer den ganzen Tag vor anderen Programmen sitzt, sieht nicht dauernd auf die Station, und eine Mitteilung je Ereignis ist entweder zu viel oder aus. Dazwischen fehlte etwas: einmal in der Stunde, was sich getan hat.
- Unter Stationswerte → FUNK, der dritte Schalter, aus als Vorgabe. Eingeschaltet meldet sich zur vollen Stunde einer aus der Crew, reihum, mit dem, was seit dem letzten Bericht an der Station geschah und die Arbeit betrifft: wer fertig ist, wer wartet, was rot ist, was geliefert wurde, dazu wer gerade wartet und arbeitet, Konflikte und fällige Erinnerungen. Höchstens drei Sätze, wie ein Funkspruch: „Hier Vega. webshop-kasse ist fertig und wartet auf dein Review; blog-texte hängt seit einer halben Stunde an den Bildern.“
- Im Log steht er als „Schichtbericht 14:00 · von selbst“, kühl blau markiert, mit Tokens und Kosten wie jede Mission. Mit „Crew spricht“ ist er zu hören, zwischen den Quindar-Tönen; ohne Stimme kommen nur die Töne. Ist ORBIT nicht vorne, kommt eine Mitteilung mit dem ersten Satz.
- War nichts, schweigt die Crew. Kommen und Gehen allein zählt nicht, auch nicht Sonnenaufgänge, Ventile und der Bericht davor; etwas muss fertig, rot, wartend oder geliefert sein. Gefragt wird in den ersten zehn Minuten nach der vollen Stunde und frühestens eine halbe Stunde nach dem letzten Bericht oder dem Einschalten.
- Das kostet Geld, darum: nur auf Wunsch, nur mit eigenem Schlüssel, höchstens acht am Tag (um Mitternacht beginnen die acht neu), ohne Werkzeuge, mit wenig Aufwand und einer Grenze von 1000 Tokens. Mit Opus ist ein Bericht etwa ein Cent, mit Sonnet oder Haiku weniger. In der Vorschau auf claude.ai gibt es ihn nicht, denn dort zahlte der Betrachter, und der hat nicht darum gebeten; in der Vorführung auch nicht, denn deren Ereignisse sind nicht echt.
- Der Bericht ist keine Mission: Er kommt nicht ins Gedächtnis des Agenten, das bleibt für das, was man ihn fragt, und er schreibt nicht „Kepler ist fertig“ ins Bordbuch. Sonst wäre jeder Bericht das Ereignis, über das der nächste berichtet. Hat der Agent gerade einen Auftrag, ist der nächste dran; bekommt er während des Berichts einen, geht keiner von beiden verloren.
- Kommt zweimal hintereinander keiner an, etwa weil der Schlüssel nicht mehr gilt, schaltet er sich ab und sagt es im Bordbuch. Ausschalten, während einer unterwegs ist, bricht ihn ab; das zählt nicht als Fehlschlag.
...
- Der Text läuft als Strom ins Log wie sonst. Das Denken sieht man dort nicht, und Tokens und Kosten nennt der Betrachter nicht; das Log zeigt deshalb keine.
- Abbrechen geht wie sonst. Verweigert man die Erlaubnis oder kann das Konto Claude dort nicht nutzen, steht das beim Auftrag und in der Crew. Der Knopf bleibt dann für den Besuch zu, gefragt wird erst nach dem Neuladen wieder. Lehnt Claude einen Auftrag ab, verschwindet auch der Teil, der schon dastand.
- Bietet der Betrachter Claude nicht an, sagt die Crew: „In dieser Vorschau kann die Crew nicht arbeiten …“. Der Schlüsseldialog kommt in der Vorschau nicht mehr von selbst; öffnet man ihn, steht dort, dass ein Schlüssel hier nicht hilft. Im Browser und im Mac-Programm bleibt alles wie vorher.
...
**Verschraubt, nicht gegossen.** Bei starkem Zoom sahen die Module aus wie lackierte Röhren und die Kopplungsknoten wie helle Tabletten mit fünf grauen Kreisen, bei Zoom 14 wie Aufkleber. Jetzt:
- **Schraubenköpfe an den Plattenkanten.** Die Schutzschilde gegen Mikrometeoriten sind Aluminiumplatten, mit Schrauben entlang ihrer Kanten auf Abstandshaltern befestigt. Jede Platte bekommt eine Reihe kleiner Köpfe an ihrer oberen und linken Kante, gut sechs Prozent des Moduldurchmessers auseinander und nicht ganz regelmäßig, denn jede Platte ist einzeln montiert. Jeder Kopf hat einen Glanzpunkt zur Sonne und einen Hauch Schatten auf der abgewandten Seite.
- **So hell wie die Röhre an seiner Stelle.** Die erste Fassung malte die Köpfe überall gleich hell, und im dunklen Gurt unten leuchteten sie wie Lichter. Jetzt folgt jeder Kopf dem Verlauf der Röhre an seiner Stelle, mit denselben Stufen wie ihr Anstrich.
- **Die Kopplungsknoten** haben zwei Flansche mit Licht- und Schattenkante, eine Fuge in der Mitte und zweimal neun Bolzen um den Umfang. Seitlich gesehen rücken sie zu den Rändern zusammen, mit dem Sinus des Umfangswinkels wie die Schutzdecken. Die Bolzen sind gewölbte Köpfe mit Glanz; ihre Helligkeit kommt aus demselben schrägen Verlauf wie der Ring. Die erste Fassung nahm dafür den Verlauf der Röhren; der Ring ist aber anders beleuchtet, und auf seiner hellen Seite standen fast schwarze Bolzen. Unter 14 Bildpunkten Breite bleibt es bei den fünf Kreisen.
- **Jede Decke altert für sich.** Ein gelblicher Fleck mit runden Ecken lag quer über den Platten der älteren Module und sah aus der Nähe aus wie ein Aufkleber. Jetzt vergilbt jede Decke einzeln, verschieden stark, und manche gar nicht; nur wo die Decken zu klein zum Zeichnen sind, bleibt der Fleck als Tönung.
- Alles davon nur, wo eine Platte mindestens etwa zwanzig Bildpunkte breit ist, und nur im gebackenen Bild: Beim Zeichnen jedes Bildes kostet es nichts. Das scharfe Nahbild backt dafür 4 bis 8 ms länger (Median aus drei, 25 statt 22 ms bei Radius 2368, 36 statt 28 bei 1200; gemessen neben einer laufenden Testreihe, also eher zu hoch).
...
- **Satellitenbilder, auf Wunsch: die Kugel als Karte.** Mit dem Knopf unten in der Zoomleiste der ganzen Erde, oder unter Stationswerte, wird die Kugel zur Karte wie bei Google Earth. Die Bilder kommen von Esri World Imagery, derselben Quelle, die God's Eye View ohne Schlüssel nimmt: Kacheln von 256 × 256 Punkten im Netz von Web-Mercator, bis Stufe 16. Dort ist ein Punkt am Äquator 2,4 Meter breit, in München 1,6.
  ...
  - **Wenn Esri nicht ankommt: die Bilder der NASA.** Im Betrachter von claude.ai läuft eine Seite in einem Rahmen, der nach seinen eigenen Regeln nichts von fremden Servern lädt, ohne sichtbaren Fehler. Die Kacheln von Esri kamen dort nie an. Die Karte blieb ein verwaschener Fleck, nachts ein paar orange Klötze, und darunter stand „Powered by Esri“ für Bilder, die nie kamen. Was neben der Seite liegt, kommt dagegen an.
    - **Die Mosaike.** Deshalb liegen im Artefakt Bilder der NASA neben der Seite, von GIBS, gemeinfrei, im selben Netz wie die Kacheln von Esri, bei Stufe 6. Das sind 2,4 Kilometer je Punkt am Äquator und 1,6 in München, viermal so fein wie die Karte unter allem.
      - Für den Tag Blue Marble mit Relief und Meeresboden, dieselbe Art wie diese Karte, damit am Rand des Fensters keine Kante entsteht.
      - Für die Nacht Black Marble 2016, daraus nur die Lichter. Sie werden genauso aufbereitet wie die scharfen Nachtlichter: warm und hell zählt, das mondbeschienene Gelände und der bläuliche Schnee bleiben dunkel, weich gesättigt.
      - Je 8 × 8 Kacheln stecken in einem Bild von 2048 × 2048 Punkten, 64 Bilder je Ebene: 21,9 MB für den Tag, 4,2 MB für die Nacht.
      - Gemacht werden sie mit `werkzeug/orbit-nasa-mosaike.py`, das 8192 Kacheln bei GIBS holt. Im Repository liegen sie nicht, nur im Artefakt.
    - **Die Kacheln daraus.** ORBIT schneidet sie sich selbst heraus. Eine Kachel der Stufe 5 sind vier der Stufe 6, auf die Hälfte verkleinert; jede liegt ganz in einem Bild, weil ein Bild immer an einer durch acht teilbaren Kachel anfängt.
      - Die Lichter kommen in den Alphakanal der Kachel, dort, wo bei Esri steht, wie viel Beschriftung einen Punkt deckt. Der Schattierer liest sie mit den Bildern der NASA dort statt aus der groben Karte. Nachts ist eine Stadt so ein Muster aus Orten und Straßen statt eines Klotzes von zehn Kilometern.
      - Deshalb gehen die Kacheln als ausgepackte Punkte (ImageData) auf den Grafikchip, nicht als Leinwand: Die speichert vormultipliziert, und wo nachts nichts leuchtet (Alpha 0), ginge das Bild des Tages verloren.
      - Die letzten vier Bildpaare bleiben entpackt im Speicher, dazu 160 fertige Kacheln statt 400 wie bei Esri, denn die kann der Browser nicht selbst vergessen.
    - **Wann.** Im Betrachter fragt ORBIT Esri gar nicht erst und nimmt gleich die Bilder der NASA. Zu erkennen ist der Betrachter an `window.claude`, das nur er bereitstellt. Anderswo wechselt ORBIT, wenn von Esri vier Absagen und kein einziges Bild kamen, also offline oder hinter einer Firewall.
    - **Wie weit.** Hineinzoomen geht dann, bis ein Punkt der Kacheln zweieinhalb Bildpunkte breit ist. In München auf einem Laptop ist das etwa 43-fach, ein Ausschnitt von gut 650 Kilometern; weiter würde es nur weicher.
      - An der Grenze steht sechs Sekunden lang ein Schild, warum es nicht weitergeht: „Schärfer geht es in der Vorschau nicht …“. Es kommt wieder, sobald man weiter hineinwill, und verblasst in der letzten Sekunde; dauernd stünde es auf der Karte, die man sich ansehen will. Auf schmalen Schirmen steht es oben unter der Kopfzeile, sonst läge es unter den Knöpfen der Zoomleiste.
      - Weiterzoomen führt dort nicht zur Station; an der Grenze einer Karte wäre das nur verwirrend.
      - Eine Stadt aus der Suche wird bis zu dieser Grenze angeflogen. War man tiefer, weil Esri eben erst ausfiel, geht der Zoom sanft zurück.
    - Die Wolken gehen früher als bei Esri, von Stufe 6 bis 7,5. An der Grenze wäre ihre Karte mit zwanzig Kilometern je Punkt nur weißer Nebel.
    - Beschriftung und Relief ruhen, denn sie kommen von Esri und AWS; das Relief steckt schon in den Bildern. Unten links steht „Bilder: NASA Blue Marble, Black Marble (GIBS)“ statt des Nachweises von Esri, und nur, wenn eine Kachel wirklich kam.
    - **Kommt gar nichts an**, auch die Mosaike nicht, steht mitten über der Bedienung ein Schild: in der Vorschau „Satellitenbilder kommen hier nicht an …“, sonst „Esri ist nicht zu erreichen (offline oder gesperrt)“. Der Schalter unter Stationswerte sagt dasselbe, und es stehen keine Nachweise für Bilder da, die nie kamen.
    - **Kein Sturm von Anfragen.** Vorher stand eine Kachel, die fehlschlug, im nächsten Bild wieder an. Ohne Netz waren das über 25.000 Anfragen in wenigen Sekunden, und scheiterten nur einzelne Kacheln, immer noch über 11.000. Jetzt wartet eine fehlgeschlagene Kachel eine halbe Minute. Kamen von Esri nur Absagen und gibt es keine Mosaike, geht alle zwanzig Sekunden eine einzige Kachel zur Probe hinaus, ob es wieder geht.
    - In der Vorschau steht über den Schaltern unter Stationswerte ein Satz: dass dort nichts von fremden Servern ankommt, dass die Satellitenbilder deshalb die der NASA sind und welche Schalter ohne Wirkung bleiben. Die scharfen Nachtlichter fragt ORBIT dort gar nicht erst an.
...
117 Tests mit Playwright, ohne echte API-Kosten. Die ganze Reihe läuft gegen einen eingefrorenen Stand. Wo „gegengeprüft“ steht, schlägt der Test gegen die alte oder eine absichtlich kaputte Fassung an.
...
- **Version 32 gegen den eingefrorenen Stand 86c73aa: 114 von 115 grün.** Rot war menue.mjs: Er zählte unter Stationswerte auch ausgeblendete Abschnitte mit, und ERINNERUNGEN ist ausgeblendet, solange es keine gibt. Jetzt zählt er nur, was man sieht; so gegen denselben Stand grün. Auf einem Foto fiel danach auf, dass auf dem Handy das Band einer fälligen Erinnerung den Kopf eines offenen Fensters samt Schließknopf verdeckte. Behoben in 191bb10; erinnern.mjs prüft das jetzt und ist gegen 86c73aa rot, gegen 191bb10 grün. Neu darin: notiz.mjs und erinnern.mjs, mit 13 und 20 kaputten Fassungen gegengeprüft, dazu eine für werkzeuge.mjs; alle 34 fielen auf. Zwei davon brachten notiz.mjs erst zum Absturz statt zu einer Meldung; seit einem robusteren Klick meldet er sie sauber und ist gegen 191bb10 weiter grün.
- **Version 31 gegen den eingefrorenen Stand f818668: 111 von 113 grün, die beiden roten allein gegen denselben Stand grün.** stationhinten.mjs maß beim Flug zu wenige Bilder (7 im Messfenster), echteiss.mjs einen Takt zu wenig. Beide liefen, während neben der Reihe noch andere Browser-Tests liefen und die Last über der Zahl der Kerne lag; allein und ohne Nachbarn waren beide grün. Neu darin: nahdetail.mjs, mit acht kaputten Fassungen gegengeprüft.
- **Version 30 gegen den eingefrorenen Stand 0fc1703: alle 112 grün**, ohne Wiederholung, auch kobold.mjs. Neu darin: crewblick, crewfragen, funkton und sprechen, zusammen mit 35 kaputten Fassungen gegengeprüft.
- **Version 29 gegen den eingefrorenen Stand 693e7b3: 107 von 108 grün.** Rot war kobold.mjs, wie schon bei Version 26 und 27: Im Messfenster zeichnete die App unter der Last der Reihe kein einziges Bild („gezeichnete Bilder: 0“). Allein gegen denselben Stand ist er grün (siehe Offene Punkte). Gegen den Endstand 4527f2a danach gezielt grün: werkzeuge, betrachter, lieferung und kobold.
- **Version 28 gegen b3bfccb: 106 von 107 grün.** Rot war lieferung.mjs mit 2,6 statt mindestens 3 Sekunden Rest, obwohl verlängert war. Die Prüfung rechnet jetzt mit der Uhr der Seite (siehe Lieferungen).
...
- **Verschraubt, nicht gegossen** (nahdetail.mjs):
  - Ganze Station: kein Nahbild, 54 Bolzen an den drei Knoten, aber keine Schraubenköpfe. Zoom 10: im scharfen Nahbild 54 Bolzen, 1163 Köpfe, 92 vergilbte Decken und kein Fleck.
...
- **Die Wache** (wache.mjs; Sitzungen, Töne, Stimme und API nachgebaut):
  - Kasse arbeitet. „wenn fertig“ antippen stellt die Wache (◉, gedrückt, Hinweis, unter Stationswerte „◉ Kasse · wenn fertig“, im Bordbuch, ◉ hinter dem Namen); noch einmal antippen hebt sie auf. Kasse wird fertig: Band „WACHE · HH:MM“ mit „Kasse ist fertig“ und der Figur, Bordbuch, der Ruf, ◷ am Namen, die Schulterleuchte blinkt bernsteinfarben (gezählte Bildpunkte vorher und nachher); die Wache ist weg, ein weiterer Wechsel meldet nichts.
  - Schon fertig, wenn die Wache kommt: nichts sofort, nichts beim Arbeiten, erst beim nächsten Fertig. „wenn sie wartet“: „Kasse wartet auf dich“. Ein Befehl endet: „npm test ist beendet“. Ein Projektordner hat keine Zeile.
  - Die Crew stellt sie (von Kepler, `station_lage` nennt sie, Funkstrecke zur Figur); beim Wechsel sagt Kepler es selbst an, ohne Ruf und ohne den allgemeinen Ton. Eine Sitzung mit „fehler“, ein Projektordner, ein unbekannter Name und die 21. Wache kommen als Fehler zurück.
  - Eigener Agent: Vega denkt, dann fertig: „Vega ist fertig“. Nach dem Neuladen ist die Wache noch da, der erste Stand meldet nichts; nach einem Tag ohne Meldung endet sie mit einem Satz im Bordbuch.
  - Der Test fand einen echten Fehler: Die Leuchte war auf dem weißen Anzug kaum zu sehen, ein fast weißer Kern mit blassem Schein (56 bernsteinfarbene Bildpunkte vorher, 55 nachher). Jetzt ist sie satt bernsteinfarben in dunkler Fassung: 55 vorher, 146 nachher.
  - Gegengeprüft mit 24 kaputten Fassungen. 21 fielen sofort auf. Zwei erst, als der Test strenger wurde: ein zweiter Ton der allgemeinen Meldung (jetzt muss es genau der Ruf sein) und die blasse Leuchte der ersten Fassung. Die Leuchte wird dabei in einem Ausschnitt gezählt, und das Zählen streut stark: Die kräftige kam auf 31 bis 116 bernsteinfarbene Bildpunkte mehr, die blasse auf −1 bis 37. Eine feste Schwelle trennt das nicht sicher, deshalb misst Version 34 die Leuchte an ihrer Stelle. Eine Fassung traf toten Code: Ein Befehl ist nie „fertig“, nur weg. Die Zeile ist für Version 34 entfernt.
- **Während du weg warst** (rueckblick.mjs):
  - 50 Minuten weg, acht Einträge: die fünf zur Arbeit in der Reihenfolge, in der sie geschahen, mit Uhrzeit und Farbe; Sonnenaufgang, Ventil und was vor dem Weggehen war, nicht. Neun Minuten weg oder nur Station: keine Karte. „Ganzes Bordbuch“ und ✕.
  - Die Crew hat die letzten Einträge in `station_lage`, und die Frage „Was ist passiert, während ich weg war?“ steht über dem leeren Feld.
  - Echte blur- und focus-Ereignisse; zwölf Einträge zeigen sieben und „5 frühere im Bordbuch“; nach 45 weiteren Einträgen hat das Bordbuch den frühen vergessen, die Karte nennt ihn trotzdem. Mit fälligem Band und auf dem Handy mit Vorführung steht die Karte darunter, ganz im Bild.
  - Der Test fand einen echten Fehler: Die Karte maß die Lage des Bands, während es noch einblendete, und lag auf dem Handy 2 Punkte im Band. Jetzt misst sie wie das Band selbst, ohne das Einblenden, und steht 8 Punkte darunter.
  - Gegengeprüft mit 16 kaputten Fassungen. 15 fielen sofort auf, eine erst mit einem neuen Fall: nur die Karte, ohne Band einer Erinnerung, und auf dem Handy muss ein offenes Fenster trotzdem darunter beginnen.
- **Notizen an Figuren** (notiz.mjs, Vorführung, API nachgebaut):
  - Tippen, während das Detailfenster jede Sekunde angeglichen wird (drei Angleichungen während des Tippens): Text und Fokus bleiben. Enter speichert, mit „von du“; ✕ ist da; unter dem Namen „✎ wartet auf Review · 2/8“, auch mit zehn Schildern im Bild; `station_lage` und `figur_detail` haben die Notiz; ⌘K mit „review“ findet die Figur. Nach dem Neuladen noch da, auch im Feld.
  - Das Fenster wechselt beim Schreiben zu einer anderen Figur: Der Text landet bei der ersten, und im Feld steht sofort die leere Notiz der zweiten. Esc verwirft und lässt das Fenster offen. ✕ löscht, auch aus dem Speicher.
  - Die Crew schreibt „Preise prüfen, dann mergen“ (von Kepler, im Fenster „geschrieben von Kepler“, Funkstrecke zur Figur), löscht mit leerem Text; ein unbekannter Name kommt als Fehler zurück. 200 Zeichen werden 80, nur Leerzeichen löscht.
  - Gegengeprüft mit 13 kaputten Fassungen.
- **Erinnerungen** (erinnern.mjs, Töne, Stimme und Mitteilungen nachgebaut; die Zeit läuft nicht ab, der Takt bekommt die Uhrzeit danach):
  - „in 15 min“ nimmt die Notiz als Text, um jetzt + 15 min; Fenster, Stationswerte, Bordbuch und Speicher stimmen. Ohne Notiz „Nach blog-texte sehen“.
  - Fällig: Band mit Zeit, Text und Figur, Bordbuch, ◷ am Namen, der Ruf (880 und 1175 Hz) genau einmal; ein zweiter Takt meldet nichts mehr. Der Knopf mit der Figur öffnet sie und fährt hin. „+10 min“ stellt sie neu, „Erledigt“ nimmt sie aus Liste und Stationswerten.
  - Neu laden: eine in der Pause fällige meldet sich einmal mit „fällig war HH:MM“, eine schon gemeldete steht sofort wieder im Band, ohne neuen Eintrag.
  - Die Crew stellt „Build prüfen“ in 20 Minuten an webshop-kasse; 0 Minuten, leerer Text und ein unbekannter Name kommen als Fehler zurück, die 21. wartende auch, und die Knöpfe im Fenster sind dann gesperrt und sagen warum. Von Kepler gestellt und mit sprechender Crew: „Hier Kepler. Erinnerung: Build prüfen.“ zwischen seinen Tönen, ohne Ruf.
  - Mitteilungen, wenn ORBIT nicht vorne ist; eine ohne Figur öffnet beim Antippen kein leeres Fenster. Handy mit Vorführung: das Band unter dem Band der Vorführung, ganz im Bild, alle Knöpfe erreichbar; im Kinomodus zu sehen.
  - Der Test fand zwei echte Fehler: Bei 0 Minuten stand im Log „in ? min“, und die Liste unter Stationswerte zog ab der neunten Erinnerung nicht mehr nach.
  - Gegengeprüft mit 20 kaputten Fassungen.
- **Die Crew sieht sichtbar nach** (crewblick.mjs, nachgebaute API, Vorführung bei Sekunde 100):
  - `station_lage`: ein Rundruf von Kepler an alle zehn anderen Figuren, Kepler selbst nicht, die nächste zuerst (gemessen an den Plätzen der Figuren). Alle zehn Strecken werden gezeichnet; bei der nächsten laufen die Punkte erst hin, dann zurück, nie umgekehrt. Danach ist nichts mehr da.
  - `figur_detail("webshop-kasse")` und `figur_detail("Niemand")`: genau eine Strecke, zu webshop-kasse. Auf dem Bogen, den die App meldet, liegt Keplers Farbe (für den Test ein deutliches Orange): 22 von 39 Punkten.
  - In der Vorschau über `sample`: `zeige_figur("Vega")` ergibt eine Strecke der Art „zeigen“ zu Vega, und ihr Detailfenster ist offen.
  - Gegengeprüft mit sieben kaputten Fassungen: keine Strecken, nicht nach Abstand geordnet, Punkte nur hin, Strecken bleiben stehen, Farbe der Bedienung statt des Agenten, in der Vorschau ohne Agent, der Agent an sich selbst. Alle sieben schlagen an. Eine achte, bei der ein unbekannter Name die erste Figur träfe, war gleichwertig: Das Werkzeug meldet dann einen Fehler, bevor eine Strecke entsteht. Sie ist nicht mitgezählt.
- **Was fragen?** (crewfragen.mjs, Vorführung, API gesperrt):
  - Sekunde 45: Ohne Fokus keine Reihe. Mit Fokus „Worauf wartet webshop-kasse?“ (bernstein) vor den allgemeinen. Ein Klick setzt sie ins Feld; das Feld verliert den Fokus dabei nicht einmal kurz, die Reihe geht zu, der Knopf ist frei, an die API geht nichts. Feld wieder leer: Reihe da. Fokus weg: Reihe weg.
  - Während die Reihe offen ist, dockt ein Frachter mit echter Ladung an: Die zweite Frage wird „Was hat der Frachter gebracht?“.
  - Sekunde 100: „Zwei ändern preis.ts – was tun?“, dann der Frachter, dann „Was ist gerade los an der Station?“. Sekunde 58: „Warum scheitert Aufrufe berechnePreis immer wieder?“. Ohne Vorführung und ohne Verbrauch nur die allgemeinen.
  - Handy mit Finger (390 breit): Die Reihe passt in den Rand und läuft seitlich, keine Frage ist breiter als 80 % des Schirms, ein Tipp füllt das Feld, und der Fokus bleibt. Die Maßstabsleiste steht 18 Punkte über der gewachsenen Fußleiste.
  - Gegengeprüft mit acht kaputten Fassungen: Reihe immer offen, ein Tipp schickt ab, Fokus geht beim Tipp verloren, ohne die Lage, zieht nicht nach, allgemeine zuerst, die lange Frage mit beiden Namen, Maßstabsleiste an der alten Stelle. Alle acht schlagen an; die Fassung ohne Fokus fiel erst auf, seit der Test zählt, wie oft das Feld den Fokus verliert.
- **Funk zum Hören** (funkton.mjs, die Tonerzeugung des Browsers nachgebaut, sie schreibt jeden Ton mit):
  - Aus als Vorgabe: Eine Antwort der Crew legt nicht einmal eine Tonerzeugung an.
  - Einschalten per Klick: Sie läuft, zur Probe 2475 Hz, reiner Sinus, 0,25 Sekunden, in 8 ms ein- und zum Ende hin ausgeblendet.
  - Kepler antwortet: 2525 Hz beim ersten Text, 2475 Hz am Ende. Die ganze Crew zugleich: ein Anfang, ein Ende.
  - Vorführung von Sekunde 25 bis 101: genau sechs Klänge, Ruf für webshop-kasse (wartet), Achtung für den Kreis, Gut für PR #12, Achtung für den Konflikt an preis.ts, Ende für webshop-kasse (fertig), Gut für die Lieferung. Die erste Sichtung klingt nicht. Die Töne haben die Frequenzen und Formen von oben.
  - Aus: nichts mehr, die Tonerzeugung ruht. Neu geladen und noch an: Vor dem ersten Klick gibt es keine Tonerzeugung und keinen Ton, nach einem Klick auf die Szene klingt es wieder.
  - Gegengeprüft mit zehn kaputten Fassungen: an als Vorgabe, ohne Anfangston, Anfang und Ende vertauscht, ohne Sperre, harter Einsatz, Meldungen stumm, Tonerzeugung ohne Klick angelegt, ausgeschaltet läuft sie weiter, nicht gemerkt, Ruf fehlt. Alle zehn schlagen an.
- **Die Crew spricht** (sprechen.mjs, Stimme und Tonerzeugung nachgebaut, beide schreiben in eine gemeinsame Zeitleiste):
  - Aus als Vorgabe: nichts gesprochen. Mit Anna (de-DE, auf dem Gerät), Samantha (en-US) und einer deutschen Netzstimme zur Wahl nimmt ORBIT Anna, und der Schalter nennt sie.
  - Kepler antwortet mit Auszeichnungen, Code und einer Liste: Die Zeitleiste ist genau 2525 Hz, „Hier Kepler. Gerade wartet niemand auf dich. Aber: pruefung:rundung und webshop-kasse ändern beide preis.ts.“, Stimme aus, 2475 Hz. Kein Anfangston schon beim ersten Text.
  - Die ganze Crew: drei Sprüche nacheinander, nie zwei zugleich, drei Tonhöhen, und das Muster Ton, Spruch, Ton genau dreimal.
  - Ohne Funkton: nur die Stimme. Aus mitten im Spruch: Die Stimme bricht ab, zwei Wartende sind gestrichen, nichts spricht weiter. Nur eine englische Stimme: nichts gesprochen, „keine deutsche Stimme auf diesem Gerät“, die Töne wie ohne Sprache.
  - Gegengeprüft mit zehn kaputten Fassungen: an als Vorgabe, jede Stimme statt nur deutscher, ganze Antwort, zwei zugleich, ohne Töne, doppelter Anfangston, Aus ohne Abbruch, gleiche Tonhöhe, Töne mit der Sperre, ohne Säubern der Auszeichnungen. Alle zehn schlagen an.
- **Werkzeuge der Crew** (werkzeuge.mjs, mit einer nachgebauten API, die ihre Antwort als Strom schickt, und einem nachgebauten Betrachter):
  - Mit Schlüssel, drei Runden. In der ersten denkt Claude (Signatur „SIG-EINS“), schreibt und ruft `station_lage`. In der zweiten ruft er `zeige_ort("Tokio")` und `figur_detail("Niemand")`, in der dritten antwortet er. Die zweite Anfrage schickt die erste Runde als `thinking(SIG-EINS)+text+tool_use` zurück, das Ergebnis als JSON mit den drei Agenten. Die dritte schickt Tokio als gezeigt und „Niemand“ als Fehler mit `is_error`. Alle Werkzeuge (inzwischen acht, der Reihe nach geprüft) haben Beschreibung und Schema, die Anleitung steht im Systemtext.
  - Steht ein Frachter am Stutzen, nennt `station_lage` seine Ladung wie das Schild: „PR #12 · webshop — Kasse: Rabattcodes“. Zuerst stand dort nur der Titel; gegen diesen Stand schlägt die Prüfung an.
  ...
  - In der Vorschau: `sample` bekommt die Werkzeuge (heute acht) mit Beschreibung, Schema und Funktion, ohne Zwischenspeicher. `station_lage` liefert die Figuren, `zeige_figur("Vega")` öffnet Vegas Detailfenster, und die Kamera folgt ihr.
...
- **Aufträge in der Vorschau** (betrachter.mjs, mit einem nachgebauten Betrachter, dessen Claude die Antwort in Stücken schickt):
  ...
  - Vorschau ohne das Angebot: Der Knopf ist zu („In der Vorschau gesperrt“), die Crew sagt warum, gefragt wird nichts.
  - Browser: wie immer, der Schlüsseldialog kommt, ohne Schlüssel bleibt der Knopf zu.
  - Abbrechen mitten in der Antwort: „Abgebrochen.“, die Figuren ruhen.
  - test.mjs, test-gedaechtnis.mjs, heutetext.mjs und sichern.mjs (die beiden letzten bauen `window.claude` für das Sichern nach) bleiben grün.
  - Gegengeprüft mit 11 kaputten Fassungen: kein Weg über den Betrachter, doch die API, ohne Rolle, ohne Gedächtnis, mit Zwischenspeicher, immer dieselbe Stufe, ohne Strom, Schlüsseldialog auch in der Vorschau, Sperre vergessen, Abbruch als Fehler, die alte Warnung. Alle 11 schlagen an.
- **Die Karte ohne fremde Server** (gesperrt.mjs, acht Fälle, alles Fremde gesperrt, der Service Worker blockiert, Flug nach München, dann sechs Rasten weiter hinein):
  - Gefunden durch einen Kommentar im Artefakt: „ich seh nix auf der Karte“. Im Betrachter kam von Esri nichts an; gegen 23 Uhr war München obendrein dunkel, mit Klötzen aus der groben Karte der Lichter.
  - Browser, Esri gesperrt, Mosaike da: 12 Absagen von Esri (die ersten acht Kacheln kommen gleichzeitig), dann die Bilder der NASA, Stufe 6, Münchens Kachel 6/34/22 geladen. Der Nachweis ist „Bilder: NASA Blue Marble, Black Marble (GIBS)“ und kein „Powered by Esri“. Der Flug kam mit Esri-Zoom (220) an und ging sanft auf die Grenze zurück (42,7-fach). Das Schild sagt „Esri ist nicht zu erreichen“, Beschriftung und Relief sagen „wirkt nur mit Esri“. Sechs Rasten weiter: Die Erde bleibt offen, der Zoom an der Grenze.
  - Browser ohne Mosaike: das Schild „nicht zu erreichen (offline oder gesperrt)“, keine Nachweise, der Schalter sagt „kommen nicht an“.
  - Wie im Betrachter (`window.claude` vorhanden), 11 Uhr UTC: keine einzige Anfrage an Esri, acht an die Mosaike (vier Paare). Der Flug endet an der Grenze oder bis 10 Prozent davor, denn er rechnet mit der Breite des Ziels, die Grenze mit der Blickmitte. Das Schild nennt die Vorschau und das Mac-Programm, der Satz über den Schaltern steht da, Beschriftung und Relief sagen „sperrt die Vorschau“. Nach 6,8 Sekunden ohne neuen Versuch ist das Schild weg.
  - Dasselbe um 23 Uhr UTC: Um München ist das Bild fein. Der mittlere Unterschied zu Nachbarn zwei Punkte weiter ist 19,6, mit den Lichtern der groben Karte 10,2; im Mittel ist es dunkler, 19,9 statt 30,9, weil die Klötze große Flächen füllten.
  - Wie im Betrachter, ohne Mosaike: das Schild zur Vorschau, keine Nachweise, keine Anfrage an Esri.
  - Kommen Kacheln von Esri an: keine einzige Anfrage an die Mosaike, der Nachweis von Esri, Zoom 220.
  - Esri antwortet, aber jede Kachel mit ungeradem x scheitert: Esri bleibt die Quelle, und in drei ruhigen Sekunden geht keine Anfrage hinaus. Ohne die halbe Minute Wartezeit waren es 11.352 Anfragen, 1.999 davon in der Ruhe.
  - Handy (390 × 844), Vorschau ohne Mosaike: Das Schild steht oben unter der Kopfzeile, innerhalb des Rands, und deckt weder die Zoomleiste noch den Zeiger zur Station. Auf dem ersten Handyfoto stand es unten, halb unter den Knöpfen der Zoomleiste.
  - Der Inhalt, im Browser gegen die rohen Kacheln von GIBS: Die Kachel 6/34/22 aus dem Mosaik weicht im Mittel um 1,0 von der rohen ab (0 bis 255, Tag) und um 2,4 von deren Lichtern; die Nachbarkachel läge bei 25 und 30. Die Kachel 5/17/11, aus vier der Stufe 6 verkleinert: 0,6 und 3,2.
  - Ruhe: Steht der Blick drei Sekunden, geht in keinem Fall eine Anfrage hinaus, auch ohne Netz und ohne Mosaike. Vorher waren es in diesem Fall über 25.000 Anfragen an Esri.
  - Gegengeprüft mit 15 kaputten Fassungen: Kacheln an der falschen Stelle im Mosaik, Stufe 5 falsch verkleinert, keine Lichter aus den Mosaiken, Tag leuchtet nachts wie Schrift, Esri in der Vorschau, keine Grenze, Nachweis von Esri, weiterzoomen führt zur Station, kein Rückzoom, keine Stufe 6, Schild bleibt stehen, kein Schild an der Grenze, keine Wartezeit für gescheiterte Kacheln, keine Probe-Regel, Schild auf dem Handy unten. Alle 15 schlagen an, jede an der Stelle, die sie treffen soll. Die Fassung ohne Wartezeit kam zuerst durch, weil kein Fall einzelne gescheiterte Kacheln hatte; dafür gibt es jetzt den Fall mit ungeradem x.
  - orte.mjs bekommt jetzt nachgebaute Kacheln von Esri: Es sperrte alles Fremde und erwartete den Flug mit Esri-Zoom; jetzt wechselte ORBIT dort richtig zu den Bildern der NASA. Danach sind orte, satellit, relief, schrift, nachtlicht, kompass, schatten, erdansicht, menue, regen, regenfilm, stationhinten und handy grün.
...
- **Lieferungen** (lieferung.mjs, Flugzeiten für den Test verkürzt):
  ...
  - Verlängert heißt: Das Ende liegt mindestens eine Liegezeit nach dem Augenblick vor dem Nachschub, gemessen in der Uhr der Seite. Vorher prüfte der Test den Rest nach 800 ms Warten an der Wanduhr; in den Reihen für Version 27 und 28 schlug er unter Last so an (2,8 und 2,6 statt mindestens 3 Sekunden), obwohl verlängert war. Ohne die Verlängerung schlägt die neue Prüfung an.
...
- **Die Wache sieht nur, was die Brücke liest.** Sitzungen kennt sie aus dem Sitzungsregister von Claude Code; ohne Register (ältere Fassungen) zählt der aus Zeitstempeln geratene Zustand, und „fertig“ kommt dann erst, wenn die Sitzung eine Weile still ist. Im Hintergrund liest die Brücke seltener, eine Wache meldet sich dort also später. Echte Sitzungen habe ich damit nicht beobachtet, nur nachgebaute.
- **Den Schichtbericht habe ich nur gegen eine nachgebaute API gesehen.** Ob die echte die Anfrage so annimmt (ohne Strom, adaptives Denken mit dem Aufwand „low“), ist nicht ausprobiert; lehnt sie zweimal ab, schaltet er sich ab und sagt im Bordbuch, warum. Wie gut drei Sätze aus den Einträgen werden, liegt am Modell; einen echten Bericht habe ich nicht gelesen. Die Zeit läuft im Test nicht ab, der Takt bekommt die volle Stunde; im Hintergrund drosselt der Browser den Takt auf einmal je Minute, das Fenster von zehn Minuten reicht dafür.
- **„Während du weg warst“ hängt an blur und focus.** Im Mac-Programm und im Browser ist das der Wechsel des Fensters; in der Vorschau auf claude.ai auch ein Klick außerhalb des Rahmens. Wie zuverlässig das Programm unter macOS diese Ereignisse meldet, ist auf keinem Mac ausprobiert.
- **Die Crew in der Vorschau ist gegen einen nachgebauten Betrachter geprüft**, nicht gegen den echten. Wie seine Rückfrage aussieht und wie lange die Stufe „complex“ vor dem ersten Wort nachdenkt (laut Beschreibung 5 bis 60 Sekunden), habe ich nicht gesehen. Drei Agenten zugleich sind drei Anfragen auf dem eigenen Konto; laut Beschreibung laufen ein paar gleichzeitig, weitere warten.
...
- **Die Erde ist echt, die Bahn nicht.**
  ...
  - Aus dem Betrachter von claude.ai heraus ist wheretheiss.at nicht erreichbar, er sperrt fremde Adressen; dann steht „nicht erreichbar“ da.
  ...
  - Die scharfen Nachtlichter sind die von 2016, nicht die von heute; neue Siedlungen fehlen. Wie hell sie herauskommen, habe ich an München geeicht, nicht gemessen. Aus dem Betrachter von claude.ai heraus ist GIBS so wenig erreichbar wie Esri; dort leuchten die Lichter aus den Mosaiken. Hier habe ich die echten Bilder mit curl geholt. Auch mit einem halben Kilometer je Punkt ist eine Stadt bei tiefem Zoom ein weicher Fleck, keine Straßen.
  - **Die Bilder der NASA liegen nur im Artefakt**, nicht im Repository (26 MB) und nicht im Mac-Programm. Fällt Esri im Browser oder im Mac-Programm aus, bleibt dort das Schild. Dass der Betrachter von claude.ai fremde Server sperrt und Dateien neben der Seite durchlässt, steht in seinen Regeln. Im echten Betrachter habe ich die Mosaike nicht gesehen, denn den kann ich von hier aus nicht öffnen; geprüft ist ein nachgebauter (window.claude vorhanden, alles Fremde gesperrt). Woran ORBIT ihn erkennt (`window.claude`), ist eine Annahme: Stellt er es einmal nicht mehr bereit, fragt ORBIT erst Esri und wechselt nach vier Absagen. Blue Marble ist ein wolkenloses Mosaik aus Aufnahmen von MODIS (bei GIBS ohne Jahr angegeben, nach meinem Wissen Blue Marble Next Generation von 2004), Black Marble zeigt die Lichter von 2016.
